...
open class QueuePlayer: NSObject {
    ...
    /// Player delegate.
    open weak var playerDelegate: QueuePlayerDelegate?

    /// Playback delegate.
    open weak var playbackDelegate: QueuePlayerPlaybackDelegate?
    ...
    open var oldAsset: AVAsset? //已经播放过的资源
    open var assets = [AVURLAsset]()
    let loadKeys = ["tracks", "playable", "duration"]
    ...
    open var bufferSizeInSeconds: Double = 6
    ...
    internal lazy var _avplayer: AVQueuePlayer = {
...
//        self.addApplicationObservers()
    ...
    //MARK: - 预加载
    @discardableResult public func prepareLoadURL(_ url: String, play: Bool = false) -> Int {
        guard let _url = URL(string: url) else {
            return -2
        }
        let index = self.assets.firstIndex(where: {$0.url.absoluteString == _url.absoluteString}) ?? -1
        if index == -1 {
            let asset = AVURLAsset(url: _url, options: .none)
            self.assets.append(asset)
            
            asset.loadValuesAsynchronously(forKeys: loadKeys) { [weak self] in
                guard let weakSelf = self else {return }
                for key in weakSelf.loadKeys {
                    var error: NSError? = nil
                    let status = asset.statusOfValue(forKey: key, error: &error)
                    if status == .failed {
                        if error?.localizedDescription.count ?? 0 > 0 {
                            weakSelf.loadErrors[asset.url.absoluteString] = error?.localizedDescription ?? ""
                        }
                        return
                    }
                }
                if !asset.isPlayable {
                    weakSelf.loadErrors[asset.url.absoluteString] = "资源不可播放"
                    return
                }
                
                let playerItem = AVPlayerItem(asset:asset)
                if !play, let _current = weakSelf._avplayer.currentItem {
                    if weakSelf._avplayer.canInsert(playerItem, after: _current) {
                        weakSelf._avplayer.insert(playerItem, after: _current)
                    }
                } else {
                    weakSelf._avplayer.replaceCurrentItem(with: playerItem)
                    if play {
                        weakSelf.playCurrentItem()
                    }
                }
            }
        }
    ...
    public func playCurrentItem() {
        self.removePlayerItemObservers()
        guard let currentItem = self._avplayer.currentItem else {
            return
        }
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(playerItemDidPlayToEndTime(_:)), name: .AVPlayerItemDidPlayToEndTime, object: currentItem)
        NotificationCenter.default.addObserver(self, selector: #selector(playerItemFailedToPlayToEndTime(_:)), name: .AVPlayerItemFailedToPlayToEndTime, object: currentItem)
    ...
    public func playURL(_ url: String) {
        let status = self.prepareLoadURL(url, play: true)
        if status >= 0 {
            let asset = self.assets[status]
            if let item = self._avplayer.items().first(where: {
                if let av = $0.asset as? AVURLAsset, av.url.absoluteString == asset.url.absoluteString {
                    return true
                }
                return false
            }) {
                self._avplayer.replaceCurrentItem(with: item)
            } else {
                let playerItem = AVPlayerItem(asset:asset)
    ...
    public func playNext() {
        self._avplayer.advanceToNextItem()
        self.playCurrentItem()
    }
    deinit {
